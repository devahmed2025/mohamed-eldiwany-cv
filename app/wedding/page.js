...
// const InputGroup = React.forwardRef(({ label, type, value, setValue, required, min, step, unit, placeholder, customKey }, ref) => (
//     <div>
//         <label className="block text-sm font-medium text-gray-300 mb-1">
//             {label} 
//             {unit && <span className="text-indigo-400 ms-1">({unit})</span>}
...
// InputGroup.displayName = 'InputGroup'; // Recommended for forwardRef

// const SelectGroup = ({ label, value, setValue, children }) => (
//     <div>
//         <label className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
//         <select
//             value={value}
...
// const TextAreaGroup = ({ label, value, setValue, rows }) => (
//     <div>
//         <label className="block text-sm font-medium text-gray-300 mb-1">{label}</label>
//         <textarea
//             value={value}
...
// // 🛑 FIX: نقل تعريف الـ Modal خارج المكون الرئيسي (PersonalTrainerApp) لمنع إعادة الإنشاء (Re-creation) مع كل تغيير في الـ State.

// // 🚨 FIX: تم إزالة `if (!isOpen) return null;` واستبدالها بالتحكم في العرض عبر الـ CSS
// const AddCustomMealModal = (({ isOpen, onClose, mealNameRef, newCustomMeal, handleNewCustomMealChange, currentMealType, currentMealOption, addCustomMeal }) => {
//     // ❌ Removed: if (!isOpen) return null;
//     return (
//         <div 
//             className={`${isOpen ? "flex" : "hidden"} fixed inset-0 bg-black bg-opacity-75 items-center justify-center z-50 p-4`}
//         >
//             <div className="bg-gray-800 p-6 rounded-xl shadow-2xl w-full max-w-lg border border-indigo-500">
...
//                     </button>
//                 </div>
//                 <p className="text-sm text-gray-400 mb-4">إضافة مكون وجبة لـ: <span className="text-indigo-300 font-semibold">{T.mealNames[currentMealType]} - {currentMealOption}</span></p>

//                 <form onSubmit={(e) => { e.preventDefault(); addCustomMeal(); }} className="space-y-4">
//                     <InputGroup 
//                         label={T.componentName} 
//                         type="text" 
//                         value={newCustomMeal.name} 
//                         // استخدام .bind() لا يزال جيداً لضمان ثبات دالة setValue
//                         setValue={handleNewCustomMealChange.bind(null, 'name')} 
//                         required 
//                         ref={mealNameRef} // Ref kept for potential future use, but focus effect removed
//                     />
...
//                             type="number" 
//                             value={newCustomMeal.weightGrams} 
//                             setValue={handleNewCustomMealChange.bind(null, 'weightGrams')} 
//                             min="1"
//                             required
//                             unit="جرام"
//                         />
...
//                             setValue={handleNewCustomMealChange.bind(null, 'calories')} 
//                             min="0" 
//                             unit="سعرة"
//                         />
//                     </div>

//                     <h4 className="text-md font-semibold text-gray-300 pt-2 border-t border-gray-700">{T.nutritionalValues} (لكل الكمية المدخلة)</h4>
//                     <div className="grid grid-cols-3 gap-4">
//                         <InputGroup 
//                             label={T.protein.split(' ')[0]} 
//                             type="number" 
//                             value={newCustomMeal.protein} 
//                             setValue={handleNewCustomMealChange.bind(null, 'protein')} 
//                             min="0" 
//                             unit="g"
//                         />
//                         <InputGroup 
//                             label={T.carbs.split(' ')[0]} 
//                             type="number" 
//                             value={newCustomMeal.carbs} 
//                             setValue={handleNewCustomMealChange.bind(null, 'carbs')} 
//                             min="0" 
//                             unit="g"
//                         />
//                         <InputGroup 
//                             label={T.fats.split(' ')[0]} 
//                             type="number" 
//                             value={newCustomMeal.fats} 
//                             setValue={handleNewCustomMealChange.bind(null, 'fats')} 
//                             min="0" 
//                             unit="g"
//                         />
//                     </div>
...
//                         <button type="button" onClick={onClose} className="py-2 px-4 bg-gray-600 hover:bg-gray-700 text-white rounded-lg transition-colors">
//                             إلغاء
//                         </button>
//                         <button type="submit" className="py-2 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg transition-colors flex items-center gap-1">
...
// // 🛑 FIX: نقل تعريف الـ Modal خارج المكون الرئيسي (PersonalTrainerApp)
// // 🚨 FIX: تم إزالة `if (!isOpen) return null;` واستبدالها بالتحكم في العرض عبر الـ CSS
// const AddCustomExerciseModal = (({ isOpen, onClose, exerciseNameRef, newExercise, handleNewExerciseChange, currentDayForExercise, addCustomExercise }) => {
//     // ❌ Removed: if (!isOpen) return null;
//     return (
//         <div 
//             className={`${isOpen ? "flex" : "hidden"} fixed inset-0 bg-black bg-opacity-75 items-center justify-center z-50 p-4`}
...
//                     </button>
//                 </div>
//                 <p className="text-sm text-gray-400 mb-4">إضافة تمرين ليوم: <span className="text-indigo-300 font-semibold">{currentDayForExercise}</span></p>
                
//                 <form onSubmit={(e) => { e.preventDefault(); addCustomExercise(); }} className="space-y-4">
//                     <InputGroup 
//                         label={T.exerciseName} 
//                         type="text" 
//                         value={newExercise.name} 
//                         setValue={handleNewExerciseChange.bind(null, 'name')} 
//                         required
//                         ref={exerciseNameRef} // Ref kept for potential future use, but focus effect removed
//                     />
...
//                             setValue={handleNewExerciseChange.bind(null, 'muscle')}
//                         >
//                             <option value="">اختر عضلة</option>
...
//                             setValue={handleNewExerciseChange.bind(null, 'sets')} 
//                             min="1"
//                         />
//                     </div>
...
//                             setValue={handleNewExerciseChange.bind(null, 'reps')} 
//                             placeholder="مثال: 10-12 أو 8"
//                         />
//                         <InputGroup 
//                             label={T.videoLink} 
//                             type="url" 
//                             value={newExercise.videoLink} 
//                             setValue={handleNewExerciseChange.bind(null, 'videoLink')} 
...
//     const [currentMealOption, setCurrentMealOption] = useState(null);
//     const [newCustomMeal, setNewCustomMeal] = useState(initialMealItem); 
//     const mealNameRef = useRef(null); // ❌ REF for Meal Modal Focus Fix (kept, but useEffect removed)
...
//         name: '', muscle: '', sets: 3, reps: '10-12', videoLink: ''
//     });
//     const exerciseNameRef = useRef(null); // ❌ REF for Exercise Modal Focus Fix (kept, but useEffect removed)
...
//     // --- Modal Focus Effects (REMOVED: The persistent DOM solution fixes this) ---
    
//     // ❌ Removed: useEffect for meal modal focus
//     /*
//     useEffect(() => {
//         if (isCustomMealModalOpen && mealNameRef.current) {
//             setTimeout(() => {
...
//     // ❌ Removed: useEffect for exercise modal focus
//     /*
//     useEffect(() => {
//         if (isCustomExerciseModalOpen && exerciseNameRef.current) {
//             setTimeout(() => {
...
//     // 🚨 FIX: استخدم useCallback لضمان ثبات دالة التغيير.
//     const handleNewCustomMealChange = useCallback((field, value) => {
//         setNewCustomMeal(prev => ({ ...prev, [field]: value }));
//     }, []);
...
//                         text-align: right;
//                     }
//                     .pdf-recommendations ul { list-style: none; padding: 0; margin: 0; }
//                     .pdf-recommendations li {
//                         background: #252525; 
//                         padding: 8px 15px; 
//                         margin-bottom: 6px; 
...
//                         text-align: center;
//                     }
//                     .target-macros { border: 2px solid #8C52FF; }
//                     .current-macros { border: 2px solid #FBC02D; }
//                     .macro-label { background-color: #1A1A1A; font-weight: 600; color: #B0B0B0; width: 25%; }
//                     .macro-value { background-color: #252525; font-weight: bold; color: #E0E0E0; width: 25%; }
...
//                         padding-right: 10px;
//                     }
//                     .pdf-meal-option h5 { font-size: 1.1em; color: #8C52FF; margin-bottom: 8px; }
//                     .pdf-meal-list { list-style: none; padding: 0; margin: 0; }
//                     .pdf-list-item { 
//                         display: flex; 
//                         justify-content: space-between;
...
//                         font-size: 0.85em;
//                     }
//                     .pdf-meal-macros { font-size: 0.8em; color: #B0B0B0; text-align: left; direction: ltr; }
...
//                         margin-left: 5px; 
//                     }
//                     .veg-water-item:last-child { border-bottom: none; }
...
//                         border-bottom: 1px dotted #444;
//                     }
//                     .day-summary { font-size: 0.8em; color: #B0B0B0; }
//                     .rest-day-map { background: #222; border: 1px solid #333; }
//                     .active-day-map { background: #252525; border: 1px solid #8C52FF; }
...
//                         align-items: center;
//                     }
//                     .rest-badge, .active-badge { font-size: 0.8em; font-weight: normal; color: #8C52FF; }
...
//                         text-align: center;
//                     }
//                     .pdf-workout-table td {
//                         padding: 8px;
//                         border: 1px solid #333;
//                         text-align: center;
//                         background-color: #1E1E1E;
//                     }
//                     .ex-name { text-align: right; direction: rtl; }
//                     .ex-video a { color: #8C52FF; text-decoration: none; }
//                     .rest-day-text { 
//                         text-align: center; 
//                         font-size: 1.1em; 
//                         color: #B0B0B0; 
...
//                     }
//                     .pdf-supplement-item strong { color: #E0E0E0; }
//                     .pdf-supplement-item span[dir="ltr"] { direction: ltr; text-align: left; display: inline-block; }
...
//                                 <h2>${T.vegWater}</h2>
//                                 <div class="veg-water-box">
//                                     <div class="veg-water-item"><strong>💧 ${T.waterIntake}:</strong> ${waterIntake} لتر يومياً</div>
//                                     <div class="veg-water-item"><strong>🥦 ملاحظات الخضروات والفواكه:</strong> ${vegetablesText}</div>
//                                 </div>
//                             </div>
...
//                                 <div class="pdf-section">
//                                     <h2>${T.pdfSection5}</h2>
//                                     <div class="pdf-supplements-grid">
//                                         ${renderSupplements()}
//                                     </div>
//                                 </div>
//                             ` : ''}
...
//                             <div class="pdf-trainer-info page-break">
//                                 <img src="${TRAINER_PHOTO_CLOSING}" alt="${trainerInfo.nameAr}" />
//                                 <h3>${trainerInfo.nameAr}</h3>
//                                 <p>${trainerInfo.title}</p>
//                                 <p dir="ltr">${trainerInfo.phone} | ${trainerInfo.email}</p>
//                                 <p style="margin-top: 15px; font-style: italic;">
//                                     نتمنى لكم بالتوفيق والنجاح في رحلة اللياقة البدنية.
//                                 </p>
//                             </div>
...
//                     <li key={item.id} className="flex justify-between items-center bg-gray-600 p-2 rounded text-sm">
//                         <span className="flex-1 truncate">{item.name} ({item.weightGrams}g)</span>
//                         <button onClick={() => removeMealItemFromOption(mealType, option, item.id)} className="text-red-400 hover:text-red-500 ms-3" >
//                             <Trash2 className="w-4 h-4"/>
//                         </button>
//                     </li>
//                 ))}
...
//                 disabled={day === T.restDay && workoutProgram[day].length === 0}
//             >
//                 <Plus className="w-4 h-4 ms-1"/> {T.addExercise}
//             </button>
//         </div>
//     );
...
//                     <div className="flex justify-between mb-6">
//                         <h2 className="text-3xl font-bold text-white flex items-center gap-2">
//                             <Lock className="w-7 h-7 text-indigo-400" /> {T.login}
//                         </h2>
//                     </div>
//                     <form onSubmit={(e) => { e.preventDefault(); handleLogin(); }} className="space-y-6">
//                         <div>
...
//     // Main Dashboard (Dark Mode + Background Image)
//     return (
//         <div className="min-h-screen bg-gray-900 text-gray-100 rtl-container custom-scrollbar-dark">
//             <AddCustomMealModal 
//                 isOpen={isCustomMealModalOpen}
//                 onClose={() => setIsCustomMealModalOpen(false)}
//                 mealNameRef={mealNameRef}
...
//             <div className="flex">
//                 {/* Sidebar - Plans List */}
//                 <aside className={`fixed top-0 right-0 h-full w-64 bg-gray-800 p-4 border-l border-gray-700 z-30 transform transition-transform duration-300 md:translate-x-0 ${showSidebar ? 'translate-x-0' : 'translate-x-full'}`}>
//                     <div className="flex justify-between items-center mb-6">
...
//                     <button onClick={() => { resetForm(); setShowSidebar(false); }} className="w-full flex items-center justify-center py-2 px-4 mb-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg font-semibold transition-colors">
//                         <Plus className="w-5 h-5 ms-2"/> {T.newPlan}
//                     </button>

//                     <div className="space-y-3 max-h-[calc(100vh-140px)] overflow-y-auto custom-scrollbar-dark">
//                         {savedPlans.map(plan => (
//                             <div key={plan.id} className={`p-3 rounded-lg shadow-md border ${editingPlan?.id === plan.id ? 'bg-indigo-900/50 border-indigo-500' : 'bg-gray-700 border-gray-600'}`}>
//                                 <p className="font-semibold text-white truncate mb-1" title={plan.clientName}>
//                                     {plan.clientName}
//                                 </p>
//                                 <p className="text-xs text-gray-400 mb-2">
//                                     المدة: {plan.programPeriod === 'custom' ? `${plan.customPeriod} ${T.weeks}` : plan.programPeriod}
//                                 </p>
//                                 <div className="flex gap-2">
//                                     <button onClick={() => { loadPlanForEdit(plan); }} className="flex-1 flex items-center justify-center py-1 px-2 bg-indigo-500 hover:bg-indigo-600 text-white text-xs rounded-md" title={T.edit} >
//                                         <Edit2 className="w-4 h-4 ms-1"/> {T.edit}
//                                     </button>
//                                     <button onClick={() => deletePlan(plan.id)} className="py-1 px-2 bg-red-600 hover:bg-red-700 text-white text-xs rounded-md flex items-center" title={T.delete} >
//                                         <Trash2 className="w-4 h-4"/>
//                                     </button>
//                                 </div>
//                             </div>
...
//                         </div>
//                         <div className="flex items-center gap-3">
//                             <button onClick={handleLogout} className="flex items-center gap-2 py-2 px-3 bg-red-600 hover:bg-red-700 text-white rounded-lg font-semibold transition-colors shadow-md" title={T.logout} >
//                                 <Lock className="w-4 h-4"/> <span className="hidden sm:inline">{T.logout}</span>
//                             </button>
//                         </div>
//                     </header>
...
//                     {/* Main Action Buttons */}
//                     <div className="flex flex-wrap gap-4 mb-8">
//                         <button onClick={savePlan} className="flex-1 flex justify-center items-center py-3 px-6 bg-green-600 hover:bg-green-700 text-white font-bold rounded-xl shadow-lg transition-colors min-w-[200px]" >
//                             <Save className="w-5 h-5 ms-2"/> {editingPlan ? T.update : T.save}
//                         </button>
//                         <button onClick={generatePDF} className="flex-1 flex justify-center items-center py-3 px-6 bg-indigo-600 hover:bg-indigo-700 text-white font-bold rounded-xl shadow-lg transition-colors min-w-[200px]" >
//                             <Download className="w-5 h-5 ms-2"/> {T.download}
//                         </button>
//                     </div>
...
//                     {/* Tab Navigation */}
//                     <div className="flex border-b border-gray-700 mb-6">
//                         <button onClick={() => setActiveTab('client')} className={`px-6 py-3 font-semibold transition-colors flex items-center gap-2 ${activeTab === 'client' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400 hover:text-white'}`}>
//                             <User className="w-5 h-5"/> {T.clientInfo}
//                         </button>
//                         <button onClick={() => setActiveTab('diet')} className={`px-6 py-3 font-semibold transition-colors flex items-center gap-2 ${activeTab === 'diet' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400 hover:text-white'}`}>
//                             <Utensils className="w-5 h-5"/> {T.dietPlan}
//                         </button>
//                         <button onClick={() => setActiveTab('workout')} className={`px-6 py-3 font-semibold transition-colors flex items-center gap-2 ${activeTab === 'workout' ? 'text-indigo-400 border-b-2 border-indigo-400' : 'text-gray-400 hover:text-white'}`}>
//                             <Dumbbell className="w-5 h-5"/> {T.trainingPlan}
//                         </button>
//                     </div>
...
//                     {activeTab === 'client' && (
//                         <div className="space-y-8">
//                             <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 bg-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700">
//                                 {/* Left/Middle Columns: Inputs */}
//                                 <div className="lg:col-span-2 space-y-4">
//                                     <h2 className="text-2xl font-bold text-indigo-400 mb-4 border-b border-gray-700 pb-3 flex items-center gap-2">
//                                         <User className="w-6 h-6"/> {T.clientInfo}
//                                     </h2>
//                                     <InputGroup 
//                                         label={T.clientName} 
...
//                                     <MetricCard 
//                                         label={T.currentCals} 
//                                         value={getTotalMacros.totalCals} 
//                                         unit="Kcal"
//                                     />
//                                     <MetricCard 
//                                         label={T.currentProtein} 
//                                         value={getTotalMacros.totalProtein} 
//                                         unit="g"
//                                     />
//                                 </div>
//                             </div>
...
//                             <div className="bg-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700">
//                                 <h2 className="text-2xl font-bold text-indigo-400 mb-6 border-b border-gray-700 pb-3 flex items-center gap-2">
//                                     <Utensils className="w-6 h-6"/> {T.dietPlan.split('2. ')[1]}
//                                 </h2>
//                                 <div className="space-y-8">
//                                     {MEAL_TYPES.map(mealType => (
//                                         <div key={mealType} className="border border-gray-700 p-4 rounded-xl">
//                                             <h3 className="text-xl font-bold text-white mb-4 flex items-center gap-2">
//                                                 <CornerDownRight className="w-5 h-5 text-indigo-400"/> {T.mealNames[mealType]}
//                                             </h3>
//                                             <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//                                                 {MEAL_OPTIONS.map(option => (
//                                                     <div key={option}>
//                                                         {renderMealOption(mealType, option)}
//                                                     </div>
//                                                 ))}
//                                             </div>
//                                         </div>
...
//                                     />
//                                     <TextAreaGroup
//                                         label="الخضروات والفواكه المتاحة/المفضلة"
//                                         value={vegetablesText}
//                                         setValue={setVegetablesText}
//                                         rows="4"
//                                     />
//                                 </div>
...
//                                                             }}
//                                                             placeholder={T.timing}
//                                                             className="bg-transparent text-gray-400 focus:outline-none focus:ring-0 w-32"
//                                                         />
//                                                         {WEEKDAY_NAMES.map(day => (
//                                                             <label key={day} className="flex items-center text-xs cursor-pointer">
//                                                                 <input
//                                                                     type="checkbox"
//                                                                     checked={sup.days.includes(day)}
//                                                                     onChange={(e) => {
//                                                                         const newSups = [...selectedSupplements];
//                                                                         const days = newSups[index].days;
//                                                                         if (e.target.checked) {
//                                                                             newSups[index].days = [...days, day];
//                                                                         } else {
//                                                                             newSups[index].days = days.filter(d => d !== day);
//                                                                         }
//                                                                         setSelectedSupplements(newSups);
//                                                                     }}
//                                                                     className="w-3 h-3 text-indigo-600 bg-gray-700 border-gray-600 rounded focus:ring-indigo-500 ms-1"
//                                                                 />
//                                                                 {day}
//                                                             </label>
//                                                         ))}
//                                                     </div>
//                                                 </div>
//                                                 <button onClick={() => setSelectedSupplements(selectedSupplements.filter((_, i) => i !== index))} className="text-red-400 hover:text-red-500 ms-3">
//                                                     <Trash2 className="w-4 h-4"/>
//                                                 </button>
//                                             </div>
//                                         ))}
//                                         <button onClick={() => setSelectedSupplements([...selectedSupplements, { name: '', dosage: '', timing: '', days: [] }])} className="w-full py-2 bg-indigo-500 hover:bg-indigo-600 text-white rounded-lg flex items-center justify-center transition-colors mt-4">
//                                             <Plus className="w-5 h-5 ms-2"/> {T.addSupplement}
//                                         </button>
//                                     </div>
//                                 </div>
//                             </div>
...
//                         <div className="bg-gray-800 p-6 rounded-2xl shadow-xl border border-gray-700">
//                             <h2 className="text-2xl font-bold text-indigo-400 mb-6 border-b border-gray-700 pb-3 flex items-center gap-2">
//                                 <Dumbbell className="w-6 h-6"/> {T.trainingPlan.split('3. ')[1]}
//                             </h2>
//                             <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
//                                 {WEEKDAY_NAMES.map(renderWorkoutDay)}
//                             </div>
//                         </div>
//                     )}
//                 </main>
...
const generatePDF = useCallback(async () => {
    ...
    // Function to convert image to Base64 (used for the smaller closing photo for reliability)
    ...
    // Fetch the closing photo and convert to Base64
    const trainerPhotoClosingBase64 = await toDataURL(TRAINER_PHOTO_CLOSING).catch(err => {
        console.error("Error fetching closing photo, using fallback URL.", err);
        return TRAINER_PHOTO_CLOSING;
    });
    ...
    const renderMeals = () => MEAL_TYPES.map(mealType => `
        <div class="pdf-meal-type">
            <h3><Utensils class="w-5 h-5"/> ${T.mealNames[mealType]}</h3>
            <div class="pdf-meal-options">
                ${MEAL_OPTIONS.map(option => `
                    <div class="pdf-meal-option">
                        <h4>${option}</h4>
                        <ul class="pdf-meal-list">
    ...
    const renderClientInfoTable = () => {
        ...
        return `
            <table class="pdf-client-table">
                <tr>
                    <td class="pdf-label">${T.clientName}:</td>
                    <td class="pdf-value">${clientName}</td>
                    <td class="pdf-label">${T.programPeriod}:</td>
                    <td class="pdf-value">${periodText}</td>
                </tr>
                <tr>
                    <td class="pdf-label">${T.weight}:</td>
                    <td class="pdf-value">${weight || 0} kg</td>
                    <td class="pdf-label">${T.targetWeight}:</td>
                    <td class="pdf-value">${targetWeight || 0} kg</td>
                </tr>
                <tr>
                    <td class="pdf-label">${T.height}:</td>
                    <td class="pdf-value">${height || 0} cm</td>
                    <td class="pdf-label">${T.goal}:</td>
                    <td class="pdf-value">${goal === 'loss' ? 'خسارة الوزن' : 'بناء العضلات'}</td>
                </tr>
                <tr>
                    <td class="pdf-label">${T.bodyFat}:</td>
                    <td class="pdf-value">${bodyFat || 0} %</td>
                    <td class="pdf-label">${T.muscleMass}:</td>
                    <td class="pdf-value">${muscleMass || 0} kg</td>
                </tr>
                <tr>
                    <td class="pdf-label">${T.bmi}:</td>
                    <td class="pdf-value">${bmi}</td>
                    <td class="pdf-label">${T.weeklyLoss}:</td>
                    <td class="pdf-value">${weeklyLoss || 0} kg</td>
                </tr>
            </table>
        `;
    };
    ...
    // Render Macros Summary as a robust HTML table 
    const renderMacrosSummaryTable = (calories, protein, carbs, fats, isTarget = true) => `
        <div class="pdf-macros-container ${isTarget ? 'target-macros' : 'current-macros'}">
            <table class="pdf-macros-table">
                <tr>
                    <td class="macro-label">${T.calories} (${isTarget ? 'الهدف' : 'من الوجبات'})</td>
                    <td class="macro-value">${calories || 0}</td>
                    <td class="macro-label">${T.protein}</td>
                    <td class="macro-value">${protein || 0}g</td>
                    <td class="macro-label">${T.carbs}</td>
                    <td class="macro-value">${carbs || 0}g</td>
                    <td class="macro-label">${T.fats}</td>
                    <td class="macro-value">${fats || 0}g</td>
                </tr>
            </table>
        </div>
        ...
        return `
            <table class="pdf-workout-table">
                <thead>
                    <tr>
                        <th style="width: 40%;">${T.exerciseName}</th>
                        <th style="width: 15%;">${T.muscle}</th>
                        <th style="width: 15%;">${T.pdfSets}</th>
                        <th style="width: 15%;">${T.pdfReps}</th>
                        <th style="width: 15%;">${T.pdfVideo}</th>
                    </tr>
                </thead>
                <tbody>
                    ${workoutProgram[day].map(ex => `
                        <tr>
                            ...
                            <td>${ex.sets}</td>
                            <td>${ex.reps}</td>
                            <td class="ex-video">${ex.videoLink ? `<a href="${ex.videoLink}" target="_blank">رابط</a>` : 'لا يوجد'}</td>
                        </tr>
                    `).join('')}
                </tbody>
            </table>
    ...
    const renderWorkoutCalendar = () => WEEKDAY_NAMES.map(day => `
        <div class="pdf-day-calendar-item ${workoutProgram[day]?.length === 0 ? 'rest-day-bg' : 'active-day-bg'}">
            <h3 class="pdf-day-title">${day} - ${workoutProgram[day]?.length === 0 ? T.restDay : T.activeDay}</h3>
            ${renderWorkoutTable(day)}
        </div>
    `).join('');
    ...
    const renderSupplements = () => selectedSupplements.map(sup => `
        <div class="pdf-supplement-item">
            <strong>${sup.name}</strong> 
            <p style="margin-top: 5px;">${T.dosage}: ${sup.dosage}</p>
            <p>${T.timing}: ${sup.timing}</p>
            <p>${T.usageDays}: <span dir="ltr">${sup.days.join(', ')}</span></p>
        </div>
    `).join('');
    ...
    // Complete htmlContent with Enhanced Dark Mode Styling (CRITICAL FIXES HERE)
    const htmlContent = `
        <!DOCTYPE html>
        <html lang="${LANGUAGE_CODE}" dir="${PDF_DIR}">
        <head>
            ...
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <style>
                /* CRITICAL: Force print color rendering across all elements */
                * { 
                    -webkit-print-color-adjust: exact !important; 
                ...
                /* 1. Ensure Trainer Background Image is on All Pages */
                .pdf-body-wrapper {
                    background-image: url('${TRAINER_PHOTO_BACKGROUND}'); 
                    background-position: center center;
                    background-size: cover;
                    background-attachment: fixed; /* Essential for multi-page PDF printing */
                    min-height: 100vh;
                    direction: rtl;
                }
                
                /* Overlay for contrast and Dark Mode */
                .pdf-background-overlay {
                    position: fixed; /* Use fixed for better print result on mobile/Safari */
                    top: 0; left: 0; right: 0; bottom: 0;
                    background: rgba(18, 18, 18, 0.95); /* Semi-transparent dark overlay */
                    z-index: 1;
                }
                ...
                body {
                    ...
                    background-color: transparent !important; /* Use wrapper background */
                    line-height: 1.6;
                    margin: 0;
                    padding: 0;
                    font-size: 10pt;
                ...
                /* 2. Page Content - Apply subtle Gradient Colors */
                .pdf-page {
                    position: relative; 
                    z-index: 2; /* Must be above overlay */
                    /* Subtle Dark Gradient for depth */
                    background: linear-gradient(to bottom, rgba(30, 30, 30, 0.98), rgba(18, 18, 18, 0.98)) !important; 
                    padding: 20px 30px;
                    margin: 0 auto;
                    max-width: 21cm;
                    min-height: 29.7cm;
                ...
                h1, h2, h3, h4, h5, h6 { 
                    color: #A78BFA !important; /* Primary Accent Color */
                    margin-top: 0; 
                    margin-bottom: 0.5em; 
                }
                ...
                .pdf-cover::before { /* Dark overlay for Cover text contrast */
                    content: '';
                    position: absolute;
                    top: 0; left: 0; right: 0; bottom: 0;
                    background: rgba(18, 18, 18, 0.5); /* Lighter overlay on cover */
                    z-index: 1;
                }
                .pdf-cover-content { position: relative; z-index: 2; padding: 20px; background: rgba(0,0,0,0.4); border-radius: 10px; }
                .pdf-cover h1 { font-size: 3em; color: #E0E0E0 !important; margin-bottom: 1em; text-shadow: 0 2px 8px rgba(0,0,0,0.8); }
                .pdf-cover h2 { font-size: 2em; color: #A78BFA !important; text-shadow: 0 2px 8px rgba(0,0,0,0.8); }
                .pdf-cover p { font-size: 1.2em; margin: 0.8em 0; color: #E0E0E0 !important; }
                .pdf-cover .prepared-by { margin-top: 5em; font-size: 1.1em; }
                .pdf-cover .trainer-name { font-size: 1.6em; color: #E0E0E0 !important; }
                ...
                /* DATA TABLES */
                .pdf-client-table, .pdf-workout-table {
                    width: 100%;
                    border-collapse: collapse;
                    margin-bottom: 20px;
                ...
                .pdf-client-table td, .pdf-workout-table th, .pdf-workout-table td {
                    padding: 10px 15px;
                    border: 1px solid #333;
                }
                .pdf-client-table .pdf-label {
                    width: 30%;
                    font-weight: bold;
                    color: #B0B0B0;
                    background: #1A1A1A;
                }
                .pdf-client-table .pdf-value {
                    width: 20%;
                    color: #E0E0E0;
                    text-align: left;
                    direction: ltr;
                ...
                /* MACROS SUMMARY - Enhanced with Gradient Border */
                .pdf-macros-container {
                    margin-bottom: 25px;
                    border-radius: 10px;
                    padding: 1px; /* The gap for the pseudo-element border */
                    break-inside: avoid;
                    background: #1A1A1A;
                }
                .target-macros {
                    ...
                    background: linear-gradient(45deg, #252525, #1A1A1A) !important;
                }
                .target-macros::before {
                    content: '';
                    position: absolute;
                    top: 0; left: 0; right: 0; bottom: 0;
                    border-radius: 10px;
                    padding: 2px;
                    background: linear-gradient(45deg, #8C52FF, #A78BFA, #4F46E5); /* Gradient Border */
                    -webkit-mask: 
                ...
                .pdf-macros-table tr, .pdf-macros-table td { border: none; }
                .pdf-macros-table .macro-label { font-weight: bold; color: #B0B0B0; text-align: right; width: 15%; }
                .pdf-macros-table .macro-value { font-size: 1.1em; font-weight: bold; color: #A78BFA; text-align: left; direction: ltr; width: 10%; }
                ...
                .pdf-meal-name { color: #E0E0E0; font-weight: 500; }
                .pdf-meal-macros { color: #B0B0B0; font-size: 0.85em; }
                
                /* VEGETABLES/WATER/SUMMARY */
                .pdf-summary-grid { 
                    display: grid; 
                    grid-template-columns: repeat(auto-fit, minmax(200px, 1fr)); 
                    gap: 15px; 
                ...
                .pdf-data-item span { color: #B0B0B0; display: block; font-size: 0.9em; }
                .pdf-data-item strong { 
                    font-size: 1.3em; 
                    color: #E0E0E0; 
                    margin-top: 5px; 
                ...
                .pdf-recommendations h4 { color: #A78BFA; border-bottom: 1px dotted #444; padding-bottom: 5px; margin-bottom: 10px; }
                .pdf-recommendations ul { list-style: none; padding: 0; margin: 0; }
                .pdf-recommendations li { margin-bottom: 8px; color: #E0E0E0; display: flex; align-items: flex-start; }
                .pdf-recommendations .bullet-point {
                ...
                /* WORKOUT STYLES - Mini Map & Calendar/Grid View */
                .pdf-workout-mini-map {
                    display: grid;
                ...
                .rest-day-bg { background: rgba(26, 26, 26, 0.9); border: 2px solid #555; }
                .active-day-bg { background: rgba(26, 26, 26, 0.9); border: 2px solid #8C52FF; }
                .pdf-day-title { color: #A78BFA !important; border-bottom: 1px solid #444; padding-bottom: 5px; margin-bottom: 10px; }

                .pdf-workout-table th { background: #1A1A1A; color: #A78BFA; font-weight: bold; }
                .pdf-workout-table td { color: #E0E0E0; }
                .pdf-workout-table tbody tr:nth-child(even) { background: #181818; }
                .pdf-workout-table tbody tr:hover { background: #252525; }

                .ex-name { text-align: right; direction: rtl; }
                .ex-video a { color: #A78BFA !important; text-decoration: none; }
                .rest-day-text { text-align: center; font-size: 1.1em; color: #B0B0B0 !important; padding: 15px; font-style: italic; }
                ...
                .pdf-supplement-item strong { color: #E0E0E0 !important; }
                .pdf-supplement-item span[dir="ltr"] { direction: ltr; text-align: left; display: inline-block; } 

                /* TRAINER INFO (End Page) - Enhanced */
                .pdf-trainer-info { 
                    margin-top: 40px; 
                    padding-top: 20px; 
                    text-align: center; 
                ...
                <div class="pdf-container">
                    ...
                    <div class="pdf-page">
                        <div class="pdf-content">
                            ...
                            <div class="pdf-section">
                                <h2>${T.macrosTarget} (الهدف)</h2>
                                ${renderMacrosSummaryTable(caloriesNeeded, targetProtein, targetCarbs, targetFats, true)}
                            </div>
                            
                            <div class="pdf-section">
                                <h2>${T.macrosTarget} (الإجمالي من الوجبات)</h2>
                                ${renderMacrosSummaryTable(getTotalMacros.totalCals, getTotalMacros.totalProtein, getTotalMacros.totalCarbs, getTotalMacros.totalFats, false)}
                            </div>
                        ...
                        <div class="pdf-content">
                            ...
                            <div class="pdf-section page-break">
                                ...
                                <div class="pdf-data-item">
                                    <span>${T.vegWater}</span>
                                    <p style="margin-top: 5px; color: #B0B0B0;">${vegetablesText}</p>
                                </div>
                                <div class="pdf-data-item" style="margin-top: 15px;">
                                    <span>${T.waterIntake}</span>
                                    <strong dir="ltr">${waterIntake} لتر</strong>
                                </div>
                            </div>
                    ...
                    <div class="pdf-page">
                        <div class="pdf-content">
                            ...
                            <div class="pdf-trainer-info page-break">
                                <img src="${trainerPhotoClosingBase64}" alt="${trainerInfo.nameAr}" />
                                <h3>${trainerInfo.nameAr}</h3>
                                <p>${trainerInfo.title}</p>
                                <p dir="ltr">${trainerInfo.phone} | ${trainerInfo.email}</p>
                                <p style="margin-top: 15px; font-style: italic;">
Wich you the best                                </p>
    ...
    try {
        ...
        if (!newWindow) {
            alert('⚠️ تم حظر النافذة المنبثقة! الرجاء السماح بالنوافذ المنبثقة لهذا الموقع وإعادة المحاولة.');
            return;
        }
        ...
        newWindow.addEventListener('load', () => {
            setTimeout(() => {
                alert(`📋 تعليمات الطباعة للحصول على أفضل نتيجة (للحفظ كملف PDF):
✅ تفعيل "رسومات الخلفية" (Background Graphics) أو "Background Images" لظهور الوضع الداكن والصور.
✅ اختيار "حفظ كـ PDF" (Save as PDF) كوجهة الطباعة.
✅ التأكد من اختيار حجم الورق A4.
✅ الهوامش: بلا (None) أو تخصيص (0) سم.

📌 في متصفح Safari (على iPhone):
يجب التأكد من أن خيار "الخلفيات" (Backgrounds) أو "رسومات الخلفية" مفعل في إعدادات الطباعة (قد يكون ضمن "إظهار التفاصيل" أو "Options").
`);
                newWindow.print();
...
}, [clientName, caloriesNeeded, targetProtein, meals, calculateBMI, calculateWeeklyWeightLoss, getPeriodInWeeks, bodyFat, muscleMass, weight, targetWeight, height, programPeriod, customPeriod, vegetablesText, waterIntake, selectedSupplements, workoutProgram, getTotalMacros, trainerInfo, goal, targetCarbs, targetFats]);
    // --- Modals and Render Functions ---
